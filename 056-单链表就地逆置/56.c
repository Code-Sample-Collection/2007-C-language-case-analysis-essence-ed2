...
} Node, *NodePtr;      // 类型重定义，即 Node 和 *NodePtr 和 struct node 等价


int main() {
    NodePtr l, p, q, r; // 用指针类型定义三个结点类型的指针
    char ch;

    l = (NodePtr)malloc(sizeof(Node)); // 分配内存空间
    ...
    while (ch != '0') { // 输入0表示输入结束
        p = (NodePtr)malloc(sizeof(Node)); // 为新输入的数据分配内存空间
        p->c = ch;
    ...
    while (q->next != NULL) {
        printf("%c-->", q->next->c); // q->next->c 表示q所指向的下一个元素的数据
